...
-- | Ekimei型の値から、表示用の文字列を返す
--
-- >>> hyoji Ekimei {kanji="テスト", kana="てすと", romaji="tesuto", shozoku="けやき"}
-- "けやき, テスト (てすと)"
hyoji :: Ekimei -> Text
hyoji Ekimei{..} = shozoku <> ", " <> kanji <> " (" <> kana <> ")"
...
-- | 駅名をローマ字から漢字に変換する
--
-- >>> romajiToKanji "tesuto" [Ekimei {kanji="テスト", kana="てすと", romaji="tesuto", shozoku="けやき"}]
-- "テスト"
-- >>> romajiToKanji "test" [Ekimei {kanji="テスト", kana="てすと", romaji="tesuto", shozoku="けやき"}]
-- ""
romajiToKanji :: Text -> [Ekimei] -> Text
romajiToKanji _ [] = ""
romajiToKanji r (Ekimei{..}:es) = if r == romaji
                                  then  kanji
                                  else romajiToKanji r es


data EkikanTree = Empty | Node EkikanTree (Text, [(Text, Double)]) EkikanTree

-- | 駅名と「駅名と距離のタプルのリスト」から、その駅までの距離を返す。
--
-- >>> assoc "後楽園" [("新大塚", 1.2), ("後楽園", 1.8)]
-- 1.8
-- >>> assoc "池袋" [("新大塚", 1.2), ("後楽園", 1.8)]
-- 1.0e15
assoc :: Text -> [(Text, Double)] -> Double
assoc ekiname es = fromMaybe inf $ lookup ekiname es


-- -- | EkikanTree と Ekikanを受け取ったら、EkikanTreeにEkikanの情報を挿入する。
-- --
-- -- >>> ekikanTree = Node (Empty ("新大塚", [("池袋", 1.0)]) Empty)
-- --
-- -- >>> insertEkikan ekikanTree {kiten = "新大塚", shuten = "茗荷谷", keiyu = "丸ノ内線", kyori = "1.2", jikan = "2"}
-- -- Node ((Node Empty ("茗荷谷", [("新大塚", 1.2)]) Empty) ("新大塚", [("茗荷谷", 1.2), ("池袋", 1.0)]) Empty)
-- insertEkikan :: EkikanTree -> Ekikan -> EkikanTree
-- insertEkikan Empty Ekikan{..} = case compare kiten shuten of
--                                   LT -> Node (Node Empty (kiten, [(shuten, kyori)]) Empty) (shuten, [(kiten, kyori)]) Empty
--                                   GT -> Node (Node Empty (shuten, [(kiten, kyori)]) Empty) (kiten, [(shuten, kyori)]) Empty
--                                   EQ -> Empty
-- insertEkikan node@(Node t1 (n, es) t2) Ekikan{..} = insertEkikan' node [(kiten, kyori),(shuten, kyori)]

-- | 2駅間の距離を取得する。
--
-- >>> getEkikanKyori "茗荷谷" "新大塚" [Ekikan {kiten="新大塚", shuten="茗荷谷", keiyu="丸ノ内線", kyori=1.2, jikan=2}]
-- 1.2
-- >>> getEkikanKyori "茗荷谷" "代々木上原" [Ekikan {kiten="新大塚", shuten="茗荷谷", keiyu="丸ノ内線", kyori=1.2, jikan=2}]
-- 1.0e15
getEkikanKyori :: Text -> Text -> [Ekikan] -> Double
getEkikanKyori k s [] = inf
getEkikanKyori k s (Ekikan{..}:es) = if k == kiten && s == shuten || s == kiten && k == shuten
                                     then kyori
                                     else getEkikanKyori k s es



-- | 2駅間が繋がっていたら、2駅間の距離を記載した文章を取得する
--
-- >>> kyoriWoHyoji "myogadani" "shinotsuka"
-- "茗荷谷駅から新大塚駅までは1.2kmです"
-- >>> kyoriWoHyoji "myogadani" "yoyogiuehara"
-- "茗荷谷駅と代々木上原駅はつながっていません"
-- >>> kyoriWoHyoji "myogadani" "yoyogishimohara"
-- "yoyogishimoharaという駅は存在しません"
-- >>> kyoriWoHyoji "myogagawa" "yoyogishimohara"
-- "myogagawaという駅は存在しません"
kyoriWoHyoji :: Text -> Text -> Text
kyoriWoHyoji k s
  | kanjik == "" = k <> "という駅は存在しません"
  | kanjis == "" = s <> "という駅は存在しません"
  | otherwise = let kyoriks = getEkikanKyori kanjik kanjis globalEkikanList
                in if kyoriks == inf
                   then kanjik <> "駅と" <> kanjis <> "駅はつながっていません"
                   else kanjik <> "駅から" <> kanjis <> "駅までは" <> T.pack (show kyoriks) <> "kmです"
  where kanjik = romajiToKanji k globalEkimeiList
        kanjis = romajiToKanji s globalEkimeiList


data Eki = Eki { namae :: Text
               , saitanKyori :: Double
               , temaeList :: [Text]
               } deriving (Show)
...
-- | Ekimei型のリストをEki型のリストに変換する。
--
-- >>> makeEkiList []
-- []
-- >>> makeEkiList [Ekimei {kanji="テスト", kana="てすと", romaji="tesuto", shozoku="けやき"}]
-- [Eki {namae = "テスト", saitanKyori = 1.0e15, temaeList = []}]
makeEkiList :: [Ekimei] -> [Eki]
-- makeEkiList [] = []
-- makeEkiList (Ekimei{..}:es) = Eki {namae = kanji, saitanKyori = inf, temaeList = []} : makeEkiList es
makeEkiList = map (\e -> Eki {namae = kanji e, saitanKyori = inf, temaeList = []})
...
-- >>> shokika "a" []
-- []
-- >>> shokika "テスト" [Eki {namae = "テスト", saitanKyori = 1.0e15, temaeList = []}]
-- [Eki {namae = "テスト", saitanKyori = 0.0, temaeList = ["テスト"]}]
shokika :: Text -> [Eki] -> [Eki]
-- shokika _ [] = []
-- shokika n (e:es) = if namae e == n
--                    then e {saitanKyori = 0} : es
--                    else e : shokika es n
shokika n = map (\e -> if namae e == n then e {saitanKyori = 0, temaeList = [n]} else e)


-- | Ekimei型のリストを、Eki型のリストに変換し初期化する。
--
-- >>> makeInitialEkiList "テスト" [Ekimei {kanji="テスト", kana="てすと", romaji="tesuto", shozoku="けやき"}]
-- [Eki {namae = "テスト", saitanKyori = 0.0, temaeList = ["テスト"]}]
makeInitialEkiList :: Text -> [Ekimei] -> [Eki]
makeInitialEkiList n = map (\e -> if kanji e == n
                                  then Eki {namae = kanji e, saitanKyori = 0, temaeList = [n]}
                                  else Eki {namae = kanji e, saitanKyori = inf, temaeList = []})



-- | Ekimei型のリストを、kanaをキーにして昇順にソートする。重複は取り除く。
--
-- >>> :{
-- seiretsu [ Ekimei {kanji="代々木公園", kana="よよぎこうえん", romaji="yoyogikouen", shozoku="千代田線"}
...
--          ]
-- :}
-- [Ekimei {kanji = "赤坂", kana = "あかさか", romaji = "akasaka", shozoku = "千代田線"},Ekimei {kanji = "表参道", kana = "おもてさんどう", romaji = "omotesandou", shozoku = "千代田線"},Ekimei {kanji = "霞ヶ関", kana = "かすみがせき", romaji = "kasumigaseki", shozoku = "千代田線"},Ekimei {kanji = "銀座", kana = "ぎんざ", romaji = "ginza", shozoku = "丸ノ内線"},Ekimei {kanji = "国会議事堂前", kana = "こっかいぎじどうまえ", romaji = "kokkaigijidoumae", shozoku = "千代田線"},Ekimei {kanji = "乃木坂", kana = "のぎざか", romaji = "nogizaka", shozoku = "千代田線"},Ekimei {kanji = "明治神宮前", kana = "めいじじんぐうまえ", romaji = "meijijinguumae", shozoku = "千代田線"},Ekimei {kanji = "代々木公園", kana = "よよぎこうえん", romaji = "yoyogikouen", shozoku = "千代田線"}]
seiretsu :: [Ekimei] -> [Ekimei]
seiretsu [] = []
seiretsu (m:ms) = insert (seiretsu ms) m
  where insert :: [Ekimei] -> Ekimei -> [Ekimei]
        insert [] n = [n]
        insert (m:ms) n = case compare (kana n) (kana m) of
                            GT -> m : insert ms n
                            EQ -> m : ms
                            LT -> n : m : ms


-- | 直前に確定した駅と未確定の駅リストから、更新処理を行った未確定の駅リストを取得する。
--
-- >>> :{
-- koushin globalEkikanList (Eki {namae = "乃木坂", saitanKyori = 2.0, temaeList = []})
-- [ Eki {namae = "湯島", saitanKyori = 3.0, temaeList = []}
-- , Eki {namae = "赤坂", saitanKyori = 3.2, temaeList = []}
-- , Eki {namae = "営団成増", saitanKyori = 3.0, temaeList = []}
-- ]
-- :}
-- [Eki {namae = "湯島", saitanKyori = 3.0, temaeList = []},Eki {namae = "赤坂", saitanKyori = 3.1, temaeList = ["赤坂"]},Eki {namae = "営団成増", saitanKyori = 3.0, temaeList = []}]
koushin :: [Ekikan] -> Eki -> [Eki] -> [Eki]
koushin ek p = map $ koushin1 p
  -- 直前に最短距離が確定した駅と未確定の駅が直接繋がっていたら、必要に応じて未確定の駅を更新する。
  where koushin1 p q = case getEkikanKyori (namae p) (namae q) globalEkikanList of
                         1.0e15 -> q
                         k      -> if saitanKyori q <= k + saitanKyori p
                                   then q
                                   else q {saitanKyori = k + saitanKyori p, temaeList = namae q : temaeList p}

emptyEki :: Eki
emptyEki = Eki {namae = "", saitanKyori = inf, temaeList = []}

-- | Eki型のリストをうけとり、「最短距離最小の駅」と「最短距離最小の駅以外からなるリスト」を返す。
--
-- >>> :{
-- saitanWoBunri [ Eki {namae = "乃木坂", saitanKyori = 2.0, temaeList = []}
--               , Eki {namae = "赤坂", saitanKyori = 3.0, temaeList = []}
--               , Eki {namae = "北千住", saitanKyori = 1.0, temaeList = []}
--               ]
-- :}
-- (Eki {namae = "北千住", saitanKyori = 1.0, temaeList = []},[Eki {namae = "乃木坂", saitanKyori = 2.0, temaeList = []},Eki {namae = "赤坂", saitanKyori = 3.0, temaeList = []}])
saitanWoBunri :: [Eki] -> (Eki, [Eki])
saitanWoBunri = foldr (\e (f,es) -> if
                          | namae f == "" -> (e, es)
                          | saitanKyori e < saitanKyori f ->  (e, f:es)
                          | otherwise -> (f, e:es)) (emptyEki, [])


-- | 未確定の駅リストと駅間リストを受け取り、ダイクストラアルゴリズムに従って最短距離と最短経路が入ったリストを返す
dijkstraMain :: [Ekikan] -> [Eki] -> [Eki]
dijkstraMain _ [] = []
dijkstraMain ek es = let (f,fs) = saitanWoBunri es
                         gs = koushin ek f fs
                     in f : dijkstraMain ek gs


-- | 始点と終点を与えると、ダイクストラアルゴリズムに従って最短距離と最短経路を返す
dijkstra :: Text -> Text -> [Eki]
dijkstra shiten shuten = let ek = seiretsu globalEkimeiList
                             shitenk = romajiToKanji shiten globalEkimeiList
                             shutenk = romajiToKanji shuten globalEkimeiList
                             initial = makeInitialEkiList shitenk globalEkimeiList
                         in filter (\e -> namae e == shutenk) $ dijkstraMain globalEkikanList initial


globalEkimeiList :: [Ekimei]
globalEkimeiList = [ Ekimei {kanji="代々木上原", kana="よよぎうえはら", romaji="yoyogiuehara", shozoku="千代田線"}
...
globalEkikanList :: [Ekikan]
globalEkikanList = [ Ekikan {kiten="代々木上原", shuten="代々木公園", keiyu="千代田線", kyori=1.0, jikan=2}
                   ...
                   , Ekikan {kiten="住吉", shuten="錦糸町", keiyu="半蔵門線", kyori=1, jikan=2}
                   , Ekikan {kiten="錦糸町", shuten="押上", keiyu="半蔵門線", kyori=1.4, jikan=2}
                   , Ekikan {kiten="中目黒", shuten="恵比寿", keiyu="日比谷線", kyori=1, jikan=2}
                   ...
                   , Ekikan {kiten="築地", shuten="八丁堀", keiyu="日比谷線", kyori=1, jikan=2}
                   ...
                   , Ekikan {kiten="秋葉原", shuten="仲御徒町", keiyu="日比谷線", kyori=1, jikan=1}
